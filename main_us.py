...
if __name__ == '__main__':
    ...
    with open('./config/params.json') as file:
        ...
        for x in content:
            generate_report_us(option, x['start_date'], x['project_name'], x['table_prefix'], x['appsflyer_api_token'], x['app_id'], x['fb_app_id'], x['fb_access_token'], x['admob_app_id'], x['platform'], x['geo_country'], x['contain_roi'], x['send_partner_email'], x['extra_date'])
        file.close()
