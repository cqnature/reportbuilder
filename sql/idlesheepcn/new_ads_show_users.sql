SELECT
  C.af_ad_scene,
  C.ad_show_user_count,
  D.new_user_count,
  C.ad_show_user_count/D.new_user_count AS daily_ad_show_user_percent
FROM (
  SELECT
    af_ad_scene,
    COUNT(DISTINCT user_pseudo_id) AS ad_show_user_count
  FROM (
    SELECT
      A.user_pseudo_id,
      B.af_ad_scene
