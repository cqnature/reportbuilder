...
def generate_dau_ads_report(query_config, date):
...
class Report(BaseReport):
    ...
    def do_generate(self):
        ...
        with open(self.output_filepath, mode='w+') as out:
            ...
            for single_date in self.extra_date:
                self.generate_dau_ads_report_at_date(report_lines, single_date)
            lately_date = max(
    ...
    def generate_dau_ads_report_at_date(self, report_lines, date):
        print("generate_dau_ads_report_at_date ", date)
        with open(self.etc_filepath) as file:
            daily_usercount = self.get_daily_count(date)
            if daily_usercount == 0:
                return
            ads_view_count_results = self.get_result(
                "DAU用户观看广告次数.sql", date)
            if len(ads_view_count_results) == 0:
                return
            ads_view_user_results = self.get_result(
                "DAU用户观看广告用户.sql", date)

            line_string = ""
            line_string += "{0},".format(Date(date).formatmd())
            line_string += "{0},".format(daily_usercount)
