    ...
    def do_generate(self):
        ...
        with open(self.output_filepath, mode='w+') as out:
            ...
            for single_date in Date(self.start_date).rangeto(self.end_date, True):
                self.generate_day_stage_report_at_date(
    ...
    def generate_day_stage_report_at_date(self, report_lines, date):
        ...
        with open(self.etc_filepath) as file:
            ...
            for row in signup_day_progress_results:
                ...
                signup_base_data[3] = 100 * \
                    float(row.user_count)/float(firstopen_usercount)
            tmp_base_datas = []
            for row in signup_day_progress_results:
                tmp_base_data = [row.rebirth, row.level, row.user_count,
            ...
            first_level_usercount = firstopen_usercount - \
                sum(t[2] for t in tmp_base_datas)
            stage_config_maps[0][1][2] = first_level_usercount
            stage_config_maps[0][1][3] = 100 * \
                float(first_level_usercount)/float(firstopen_usercount)
            ...
            for single_date in Date(date).rangeto(self.get_retention_date(date)):
                ...
                if currentDayIndex == 1:
                    ...
                    stage_config_maps[0][1][3] = 100 * \
                        float(first_lost_usercount)/float(firstopen_usercount)
                    ...
                    for k in range(len(current_lost_datas)):
                        ...
                        lost_day_progress_lines.append("{0}-{1},{2},{3:.2f}%,".format(
                            data[0], data[1], data[2] - base_data[2], data[3] - base_data[3]))
