...
def generate_lost_reset_report(query_config, date):
...
class Report(BaseReport):
    def __init__(self, query_config, date):
        ...
        self.etc_filename = '流失用户领悟次数.csv'
        country_string = self.query_config.geo_country
        platform_string = self.query_config.platform
        self.output_filename = "{0}-{1}-Day{2}-LostUser-Reset-{3}.csv".format(
            ...
            if lost_day == 1:
                ...
                if Date(single_date).between(self.end_date) > 0:
                    ...
                    lost_day_results = self.get_result(
                        "流失用户领悟次数.sql", date, single_date)
                    for row in lost_day_results:
                        lost_base_data = [row.reset_cnt, row.user_count,
                ...
                if Date(single_date).between(self.end_date) > 0:
                    ...
                    lost_day_results = self.get_result(
                        "流失用户领悟次数.sql", date, single_date)
                    for row in lost_day_results:
                        lost_day_data = [row.reset_cnt, row.user_count,
                                         100*float(row.user_count)/float(firstopen_usercount)]
