...
class Entry(BaseEntry):
    def __init__(self, option, *parameter):
        ...
        self.detail_email.extend(['eli@peakxgames.com', 'kinder@peakxgames.com',
                                 'near@peakxgames.com', 'nero@peakxgames.com', 'bear@peakxgames.com'])
    ...
    def do_generate_report(self):
        ...
        if self.option & ReportFlag.retention_level:
            report_filepaths.extend(generate_retentionplant_report(
                self.query_config, self.start_date))
        ...
        if self.option & ReportFlag.retention_ads:
            report_filepaths.extend(generate_retention_ads_report(
                self.query_config, self.start_date))
        if self.option & ReportFlag.total_ads:
            report_filepaths.extend(generate_total_ads_report(
                self.query_config, self.start_date))
        ...
        if self.option & ReportFlag.retention_ads_count:
            report_filepaths.extend(generate_retention_ads_count_report(
                self.query_config, self.start_date))
