...
class Report(BaseReport):
    def __init__(self, query_config, date):
        ...
        self.output_filename = "{0}-{1}-Day1-Ad-Area{2}-Level-{3}.csv".format(
            country_string, platform_string, area_id + 1, self.end_date)
    ...
    def generate_retention_ads_report_at_date(self, report_lines, date):
        ...
        with open(self.etc_filepath) as file:
            ...
            level_view_count_results = self.get_result(
                "level_ads_view_of_retention_users.sql", date, date, area_id)
