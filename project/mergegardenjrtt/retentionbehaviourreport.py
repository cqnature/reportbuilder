...
class Report(BaseReport):
            ...
            with open(filepath, mode='w+') as out:
                ...
                for single_date in Date(self.start_date).rangeto(self.end_date, True):
                    if single_date == self.end_date:
                        continue
                    self.generate_retentionbehaviour_report_at_date(
    ...
    def generate_retentionbehaviour_report_at_date(self, report_lines, date, level):
        ...
        with open(self.etc_filepath) as file:
            ...
            # 次日留存用户数
            retention_usercount = self.get_retention_count(
                date, Date(date).adddays(add_day))
            lines[2] = lines[2].strip().format(retention_usercount, 100 *
                                               float(retention_usercount)/float(firstopen_usercount))
            behaviour_results = self.get_result(
                "behaviour_of_retention_users.sql", date, Date(date).adddays(add_day), level)
            level_retention_usercount = sum(1 for _ in behaviour_results)
            lines[3] = lines[3].strip().format(level, level_retention_usercount,
                                               100*float(level_retention_usercount)/float(firstopen_usercount))
