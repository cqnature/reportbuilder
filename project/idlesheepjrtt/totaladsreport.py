...
class Report(BaseReport):
    ...
    def generate_total_ads_report_at_date(self, report_lines, date):
        ...
        if date == self.start_date:
            with open(self.etc_filepath) as file:
                ...
                for single_date in Date(date).rangeto(self.end_date, True):
                    append_line(report_lines, 0, lines[2].strip().format(
                        Date(date).between(single_date) - 1))
        ...
        for single_date in Date(date).rangeto(self.end_date, True):
            ...
            append_line(report_lines, index, "{0},{1},{2:.2f},".format(
                user_count, view_count, average_view_count))
