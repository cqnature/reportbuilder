        ...
        for rebirth in range(0, 2):
            for level in range(1, 5):
                if rebirth == 1 and level == 1:
                    continue
                filepath = self.append_output_filename(
                    '_rebirth_' + str(rebirth) + '_level_' + str(level))
        ...
        with open(self.etc_filepath) as file:
            ...
            if rebirth == 0 and level == 1:
                behaviour_results = self.querysql.get_result(
                    "behaviour_of_lost_users_0.sql", date, cur_date.adddays(add_day), cur_date.adddays(add_day - 1))
            else:
                behaviour_results = self.querysql.get_result("behaviour_of_lost_users.sql", date, cur_date.adddays(
                    add_day), cur_date.adddays(add_day - 1), rebirth, level)
            lost_usercount = sum(1 for _ in behaviour_results)
            lines[3] = lines[3].strip().format(rebirth, level, lost_usercount,
