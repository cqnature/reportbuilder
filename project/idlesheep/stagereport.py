    ...
    def generate_stage_report_at_date(self, report_lines, date):
        ...
        with open(self.etc_filepath) as file:
            ...
            signup_day_progress_results = self.get_result(
                "stage_progress_of_signup_users.sql", date)
            signup_day_progress_lines[0] = signup_day_progress_lines[0].format(
                Date(date).formatmd())
            signup_day_progress_lines[2] = signup_day_progress_lines[2].format(
                firstopen_usercount, 100)
            signup_base_datas = []
            for row in signup_day_progress_results:
                signup_base_data = [row.rebirth, row.level, row.user_count,
                                    100*float(row.user_count)/float(firstopen_usercount)]
                signup_base_datas.append(signup_base_data)
            first_level_usercount = firstopen_usercount - \
                sum(t[2] for t in signup_base_datas)
            signup_base_datas.insert(
                0, [0, 1, first_level_usercount, 100*float(first_level_usercount)/float(firstopen_usercount)])
            ...
            if Date(single_date).between(self.end_date) > 0:
                ...
                for row in lost_day_results:
                    lost_base_data = [row.rebirth, row.level, row.user_count,
                                      100*float(row.user_count)/float(firstopen_usercount)]
                ...
                lost_base_datas.insert(
                    0, [0, 1, first_lost_usercount, 100*float(first_lost_usercount)/float(firstopen_usercount)])

                lost_base_usercount = current_lost_usercount
                lost_day_progress_lines[0] = lost_day_progress_lines[0].format(
                    Date(date).formatmd())
                lost_day_progress_lines[2] = lost_day_progress_lines[2].format(
                    firstopen_usercount, 100)
