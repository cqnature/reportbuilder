...
class QuerySql:
    ...
    def create_cache_folder(self, filename, *parameter):
        ...
        folders = (self.config.cache_folder, project_name, geo_country, platform, sql_file) + parameter
        path = '.'
    ...
    def get_cache(self, filename, *parameter):
        path = self.create_cache_folder(self, filename, *parameter)
        print('get cache in file path: ', path)
    ...
    def set_cache(self, filename, *parameter, rows):
    ...
    def get_result(self, filename, *parameter):
        ...
        if result != None:
            return result
        filepath = os.path.join(self.project_config.sql_path, filename)
        if os.path.exists(filepath) and os.path.isfile(filepath):
            ...
            with open(filename) as file:
                ...
                file.close()
            rows = list(results)
            self.set_cache(filename, *parameter, row)
            return rows
        else:
